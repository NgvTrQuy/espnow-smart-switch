...
// Cùng layout với Gateway (relay/command) — chỉ đổi tên cho rõ nghĩa
struct message {
    uint8_t id = ID;
    uint8_t data;   // slot: 1=relay, 2=current, 3=power, 4=energy
    uint16_t value;
};
...
public:
    ...
    void sendData(uint8_t data, uint16_t value);// Gửi dữ liệu đến Server
};
